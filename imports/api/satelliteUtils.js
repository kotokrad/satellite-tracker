...
export function getPosition(lines, shortDate) {
...
function nextPass(lines, observer, date, interval) {
  ...
  let endDate = moment(date).add(1, 'd');
  let pass = {};
...
export function getPassList(lines, observerCoords, startDate, endDate) {
  ...
  const unfilteredPassList = roughPasses.map(roughPass => {
    ...
    const localMaxElevation = localPasses.reduce((mel, current) => {
      return current.elevation > mel ? current.elevation : mel;
    }, 0);
    return {
